...
namespace PhoneInfo.API.Tests
{
	...
	public class CatalogTests
	{
		...
		public async Task GetBranchAsync()
		{
			(string html, _) = await _catalogService.GetProductByBrandAsync("acer-phones-59");
		...
		{
			(string html, _) = await _catalogService.GetProductDetailAsync("iphone_13_promax");
